...
async function send(to, subject, html, text) {
  ...
  try {
    ...
    // Don't throw — email failure should never break the API response
  }
}
...
<body>
  <div class="wrapper">
    <div class="header"><h1>Enlight Lab</h1><span>Next-Gen AI-Powered Solutions</span></div>
    <div class="body">${content}</div>
    <div class="footer">© 2026 EnlightLab · contact@enlightlab.com<br>This email was sent because you interacted with our website.</div>
  </div>
</body>
</html>`;
}
...
async function sendBookingConfirmation(booking) {
  const html = baseTemplate(`
    ...
    <p>Hi ${booking.name},</p>
    <p>Your AI demo call with EnlightLab has been scheduled. Our AI voice agent will call you at your chosen time.</p>
    ...
    <hr class="divider">
    <p style="font-size:.85rem;color:#6B7A9E">Need to reschedule? Just reply to this email.</p>
  `);
  await send(booking.email, `✅ AI Demo Call Confirmed — ${booking.slotLabel} at ${booking.slotTime}`, html);
}

// ─── POST-CALL SUMMARY (to lead) ────────────────────────────
async function sendPostCallSummary({ booking, duration, recordingUrl, transcript, disconnectionReason }) {
  ...
  const html = baseTemplate(`
    ...
    ${transcript ? `<p><strong>Transcript excerpt:</strong></p><div class="detail-box" style="font-size:.85rem;font-family:monospace;white-space:pre-wrap">${transcript.slice(0, 1200)}${transcript.length > 1200 ? '…' : ''}</div>` : ''}
...
async function sendContactConfirmation(lead) {
  const html = baseTemplate(`
    ...
    <p>Thanks for reaching out to EnlightLab. Our team will get back to you within 24 hours.</p>
    <div class="detail-box">
    ...
    <p>In the meantime, feel free to try our AI voice agent demo.</p>
...
async function sendInternalNotification(data, type) {
  ...
  if (type === 'outbound_call') {
    ...
    subject = `🔔 New Contact Lead — ${data.fullName} (${data.service})`;
    html = baseTemplate(`
